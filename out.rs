...
pub mod ast {
    ...
    impl AstNode for Param {
        fn get_range(&self) -> Range {
            match self {
                ...
                Param { name, .. } => Range::from((&name.get_range(), &name.get_range())),
            }
        }
    }
    ...
    impl TreeDisplay for Param {
        fn num_children(&self, _cfg: &Config) -> usize {
            match self {
                Param { ty, name } => 0 + { (if let Some(_) = ty { 1 } else { 0 }) + 0 } + 1,
                _ => 0,
            }
        }
        fn child_at(&self, index: usize, _cfg: &Config) -> Option<&dyn TreeDisplay> {
    ...
    impl<T: AstNode> AstNode for EnclosedPunctuationList<T> {
        fn get_range(&self) -> Range {
            match self {
                EnclosedPunctuationList { open, close, .. } => {
    ...
    impl<T: AstNode> TreeDisplay for EnclosedPunctuationList<T> {
        fn num_children(&self, _cfg: &Config) -> usize {
            match self {
                EnclosedPunctuationList { open, items, close } => 0 + 1 + 1 + 1,
                _ => 0,
            }
        }
        fn child_at(&self, index: usize, _cfg: &Config) -> Option<&dyn TreeDisplay> {
            match self {
                EnclosedPunctuationList { open, items, close } => {
                    {
                        let mut ind = 0;
                        if let Some(v) = Some(open) {
                            if index == ind {
                                return Some(v);
                            }
                            ind += 1;
                        }
                        if let Some(v) = Some(items) {
                            if index == ind {
                                return Some(v);
                            }
                            ind += 1;
        ...
        fn num_children(&self, _cfg: &Config) -> usize {
            match self {
                ...
                Type::Ref {
                    ...
                    mutable,
                    base_type,
                } => 0 + 1 + { (if let Some(_) = base_type { 1 } else { 0 }) + 0 },
                Type::Array(_a0) => 0 + 1,
                Type::Union(_a0) => 0 + 1,
                Type::Tuple(_a0) => 0 + 1,
                Type::Generic { base_type, list } => {
                    0 + { (if let Some(_) = base_type { 1 } else { 0 }) + 0 } + 1
                }
    ...
    impl AstNode for Type {
        fn get_range(&self) -> Range {
            match self {
                ...
                Type::Float { token, .. } => Range::from((&token.get_range(), &token.get_range())),
                Type::Fixed { token, .. } => Range::from((&token.get_range(), &token.get_range())),
                Type::Boolean(_a0, ..) => Range::from((&_a0.get_range(), &_a0.get_range())),
                Type::Char { token, .. } => Range::from((&token.get_range(), &token.get_range())),
                Type::Ident(_a0, ..) => Range::from((&_a0.get_range(), &_a0.get_range())),
                Type::Ref {
                    ref_token,
                    base_type: Some(base_type),
                    ..
                } => Range::from((&ref_token.get_range(), &base_type.get_range())),
                Type::Ref { ref_token, .. } => {
                    Range::from((&ref_token.get_range(), &ref_token.get_range()))
                }
                Type::Array(_a0, ..) => Range::from((&_a0.get_range(), &_a0.get_range())),
                Type::Union(_a0, ..) => Range::from((&_a0.get_range(), &_a0.get_range())),
                Type::Tuple(_a0, ..) => Range::from((&_a0.get_range(), &_a0.get_range())),
                Type::Generic {
                    base_type: Some(base_type),
                    list,
                    ..
                } => Range::from((&base_type.get_range(), &list.get_range())),
                Type::Generic { list, .. } => Range::from((&list.get_range(), &list.get_range())),
                Type::Expression(_a0, ..) => Range::from((&_a0.get_range(), &_a0.get_range())),
                Type::Function { parameters, .. } => {
                    Range::from((&parameters.get_range(), &parameters.get_range()))
                }
                Type::Option {
                    base_type: Some(base_type),
                    question,
                    ..
                } => Range::from((&base_type.get_range(), &question.get_range())),
                ...
                Type::Result { error, .. } => Range::from((&error.get_range(), &error.get_range())),
                Type::Struct(_a0, ..) => Range::from((&_a0.get_range(), &_a0.get_range())),
            }
        }
    }
    ...
    impl TreeDisplay for ParsedTemplate {
        fn num_children(&self, _cfg: &Config) -> usize {
            match self {
                ParsedTemplate::String(_a0) => 0 + 1,
                ParsedTemplate::Template(_a0, _a1, _a2) => 0 + 1 + 1 + 1,
                _ => 0,
            }
        }
        fn child_at(&self, index: usize, _cfg: &Config) -> Option<&dyn TreeDisplay> {
            match self {
                ParsedTemplate::String(_a0) => {
                    {
                        let mut ind = 0;
                        if let Some(v) = Some(_a0) {
                            if index == ind {
                ...
                ParsedTemplate::Template(_a0, _a1, _a2) => {
                    {
                        let mut ind = 0;
                        if let Some(v) = Some(&**_a0) {
                            if index == ind {
    ...
    impl NodeDisplay for ModiferStatement {
        fn fmt(&self, f: &mut std::fmt::Formatter, _cfg: &Config) -> std::fmt::Result {
            f.write_fmt(::core::fmt::Arguments::new_v1(&["ModiferStatement"], &[]))?;
            Ok(())
        }
    }
        ...
        fn num_children(&self, _cfg: &Config) -> usize {
            match self {
                ModiferStatement {
                    ...
                    modifier_token,
                    statement,
                } => 0 + 1 + 1 + { (if let Some(_) = statement { 1 } else { 0 }) + 0 },
                _ => 0,
            }
        }
        fn child_at(&self, index: usize, _cfg: &Config) -> Option<&dyn TreeDisplay> {
            match self {
                ModiferStatement {
                    ...
                    {
                        let mut ind = 0;
                        if let Some(v) = Some(modifier) {
                            if index == ind {
                                return Some(v);
                            }
                            ind += 1;
                        }
                        if let Some(v) = Some(modifier_token) {
                            if index == ind {
                                return Some(v);
                            }
                            ind += 1;
                        }
                        if let Some(v) = statement.map_tree() {
                            if index == ind {
                                return Some(v);
                            }
                            ind += 1;
    ...
    impl NodeDisplay for Span {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>, cfg: &Config) -> std::fmt::Result {
            match &cfg.format_type {
                FormatType::Debug => f.write_fmt(::core::fmt::Arguments::new_v1(
                ...
                FormatType::Display => f.write_fmt(::core::fmt::Arguments::new_v1(
                    &["Span line: ", ", character: ", ", ", " long, token: "],
                    &[
                        ::core::fmt::ArgumentV1::new_display(&self.line_num),
                        ::core::fmt::ArgumentV1::new_display(&self.position),
